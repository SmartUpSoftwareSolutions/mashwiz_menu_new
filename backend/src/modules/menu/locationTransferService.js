...
class LocationTransferService {
  ...
  async transferLocations(branchCode, branchName, dbPool) {
    try {
      ...
      const locationData = {
        ...
        map_link:
          branch?.MAP_LINK?.trim() ||
          `https://maps.google.com/?q=${encodeURIComponent(branch?.BRANCH_NAME || branchCode)}`,
        location_order: null,
        is_open_24_7: null,
      ...
      try {
        ...
        logger.error(
          `Error finding existing location for branch ${branchCode}:`,
          findError
        );
        throw new Error(
          `Failed to check for existing location: ${findError.message}`
        );
      }
      ...
      if (existingLocation) {
        try {
          ...
          logger.error(
            `Error updating location for branch ${branchCode}:`,
            updateError
          );
      ...
      logger.error(`Error in transferLocations for branch ${branchCode}:`, {
        error: error.message,
        stack: error.stack,
        branchCode,
      });
  ...
  async syncAllLocations(branches, dbPool) {
    try {
      if (!this.prisma) {
        logger.warn("Skipping location sync - Prisma not initialized");
        return {
          success: true,
          synced: 0,
